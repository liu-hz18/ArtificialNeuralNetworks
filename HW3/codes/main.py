...
parser.add_argument('--cell', type=str, default='gru', choices=["rnn", "gru", "lstm"],
    help='Type of RNN Cell. Default: gru')
...
save_dir = f'{args.train_dir}/{args.name}_{args.cell}_{args.decode_strategy}_l={args.layers}_t={args.temperature}_p={args.max_probability}'
if not args.test:
    ...
    if not args.test:
        ...
        for epoch in range(1, args.num_epochs + 1):
            ...
            if val_ppl < best_val_ppl:
                ...
                with open(os.path.join(save_dir, 'checkpoint_%s_best.pth.tar' % args.name), 'wb') as fout:
            ...
            writer.add_scalar("valid_ppl", val_ppl, global_step=epoch)

            for example_id, sent in enumerate(samples):
                print("Example %d: " % example_id + " ".join(sent))

    else:
        model_path = os.path.join(save_dir, 'checkpoint_%s_best.pth.tar' % args.test)
        ...
        with open(os.path.join(save_dir, 'output.txt'), 'w') as fout:
