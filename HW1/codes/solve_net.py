...
def train_net(model, loss, config, inputs, labels, batch_size, disp_freq):
    ...
    for input, label in data_iterator(inputs, labels, batch_size):
        ...
        if iter_counter % disp_freq == 0:
            avg_loss, avg_acc = np.mean(loss_list), np.mean(acc_list)
            loss_log.append(avg_loss)
            acc_log.append(avg_acc)
            msg = '  Training iter %d, batch loss %.4f, batch acc %.4f' % (iter_counter, avg_loss, avg_acc)
            loss_list = []
            acc_list = []
            LOG_INFO(msg)
...
def test_net(model, loss, inputs, labels, batch_size):
    ...
    avg_loss, avg_acc = np.mean(loss_list), np.mean(acc_list)
    msg = '    Testing, total mean loss %.5f, total acc %.5f' % (avg_loss, avg_acc)
    LOG_INFO(msg)
