...
def interpolate(model, sqrt_K=8, c_min=-1.0, c_max=2.0, path='./interpolate.pdf'):
    import matplotlib.pyplot as plt
    from torchvision.utils import make_grid
    noise_a = torch.randn(1, model.latent_dim, 1, 1).to(device)
    noise_b = torch.randn(1, model.latent_dim, 1, 1).to(device)
    noise_c = torch.randn(1, model.latent_dim, 1, 1).to(device)
    noise_d = torch.randn(1, model.latent_dim, 1, 1).to(device)
